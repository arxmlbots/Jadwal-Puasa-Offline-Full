            ...
            if data["code"] == 200:
                for day_data in data["data"]:
                    ...
                    jadwal_harian = {
                        ...
                        "Imsak": timings["Imsak"][:5],  	 # Imsak tomg barangdahar sia teh
                        "Subuh": timings["Fajr"][:5],		 # Adzan subuh ini
                        "Sunrise": timings["Sunrise"][:5],	 # Matahari Terbit cuy
                        "Dzuhur": timings["Dhuhr"][:5],		 # Adzan Dzuhur
                        "Ashar": timings["Asr"][:5],		 # Adzan Ashar ini mah
                        "Sunset": timings["Sunset"][:5],	 # Matahari Terbanam cuy
                        "Maghrib": timings["Maghrib"][:5],	 # Adzan magrib tong kaluar imah
			"Buka Puasa": timings["Maghrib"][:5],	 # Jam buka puasa
                        "Isya": timings["Isha"][:5],		 # Adzan Isya ini teh
                        "Tarawih": timings["Isha"][:5],  	 # Tarawih sama dengan Isya
                        "Midnight": timings["Midnight"][:5]	 # Tengah Peting Sia teh
                    }
...
def print_table(jadwal_harian, next_sholat):
    ...
    for nama in urutan:
        ...
        if highlight:
            line = f"{Fore.CYAN}| {nama:<14} | {waktu:<6} |"
        else:
...
def get_next_sholat(jadwal_harian, sekarang, data_tahunan, tanggal_sekarang):
    ...
    # Jika semua sudah lewat, cari sholat pertama besok (Sahur)
